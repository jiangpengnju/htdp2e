...
; Equip the program from ex208 with a stop-when clause. The game ends when one of
; the columns contains HEIGHT blocks.

; It turns out that the design of stop-when handler is complex. So here are hints:
; (1) Design a function that consumes a natural number i and creates a column of i
; blocks. Use the functions to define a Landscape for which the game should stop.
...
; (3) Finally, design a function that determines a function that determines
; whether the length of any column in some given Landscape is HEIGHT.
