...
; Modify your program from ex201 so that it stops if the worm has run into the walls
; of the world. When the program stops because of this condition, it should render
; the final scene with the text "worm hit border" in the lower left of the world
; scene.

; Hint: You can use the stop-when clause in big-bang to render the last world
; in a special way.
; Challenge: Show the worm in this last scene as if it were on its way out of
; the box.
...
; Worm -> Boolean
; see if the worm w hits the wall

(check-expect (hit-wall? ex2) #f)
(check-expect (hit-wall? (make-worm 20 3 "right")) #t)
...
; Worm -> Image
; render worm onto the MT as if it were on its way out of the box.

(check-expect (render-hit-wall (make-worm 20 3 "right"))
...
(define (worm-main r)
  (big-bang
   ...
   [on-tick tock r]
   [on-key control]
   [stop-when hit-wall? render-hit-wall]))
